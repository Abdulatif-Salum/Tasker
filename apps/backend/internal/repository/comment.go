...
func(r *CommentRepository) GetCommentsByTodoID(ctx context.Context, userID string, todoID uuid.UUID) ([]comment.Comment, error) {
	  stmt := `
		...
		if err != nil{
			  return nil, fmt.Errorf("failed to execute get comment by todo id query for todo_id=%s user_id=%s: %w", todoID.String(), userID, err)
		}
...
func(r *CommentRepository) GetCommentByID(ctx context.Context, userID string, commentID uuid.UUID) (*comment.Comment, error){
	  stmt := `
		...
		if err != nil{
			  return nil, fmt.Errorf("failed to execute get comment by id for comment_id=%s, user_id=%s: %w", commentID.String(), userID, err)
		}
